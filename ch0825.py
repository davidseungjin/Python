...
8.25 LAB*: Program: Soccer team roster (Dictionaries)
...
mydic = {}
while i < 5:
    myjersey = int(input("Enter player %d's jersey number:\n" % (i+1)))
    myrating = input("Enter player %d's rating:\n" % (i+1))
    mydic[myjersey] = myrating
    print()
    i += 1
#print(mydic)
sorted_myjersey = sorted(mydic.keys())
#print(sorted_myjersey)

print("ROSTER")
i = 0
while i < len(mydic.keys()):
    print("Jersey number: %d, Rating: %s" % (sorted_myjersey[i], mydic[sorted_myjersey[i]]))
    i += 1
print()
...
while myinput != 'q':
    ...
    if myinput == 'a':
        ...
        print()
    if myinput == 'd':
        myjersey = int(input("Enter a jersey number:\n"))
        del mydic[myjersey]
        sorted_myjersey = sorted(mydic.keys())
        print()
    if myinput == 'u':
        myjersey = int(input("Enter a jersey number:\n"))
        myrating = input("Enter a new rating for player:\n")
        mydic[myjersey] = myrating
        sorted_myjersey = sorted(mydic.keys())
        print()
    if myinput == 'r':
        myrating = input("Enter a rating:\n")
        sorted_myjersey = sorted(mydic.keys())
        print("ABOVE %s" % myrating)
        i = 0
        while i < len(mydic.keys()):
            if mydic[sorted_myjersey[i]] > myrating:
                print("Jersey number: %d, Rating: %s" % (sorted_myjersey[i], mydic[sorted_myjersey[i]]))
            i += 1
        print()
